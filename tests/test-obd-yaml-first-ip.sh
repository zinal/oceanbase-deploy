...
bash -n "${ROOT}/scripts/join-empty-observer.sh"
...
grep -q "ERROR 4179" "${ROOT}/scripts/lib/common.sh"
grep -q "join_empty_observer" "${ROOT}/scripts/lib/common.sh"
grep -q "join-empty-observer.sh" "${ROOT}/scripts/deploy.sh"
grep -q "root@sys" "${ROOT}/scripts/lib/common.sh"
...
grep -q "start -s по IP ненадёжен" "${ROOT}/scripts/lib/common.sh"
