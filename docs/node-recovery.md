...
Индекс совпадает с inventory: `2` → `OBSERVER_2_*`. Zone узла считается по кругу от индекса (`scripts/lib/ob_zones.py`): `1,4,7…` → `zone1`, `2,5,8…` → `zone2`, `3,6,9…` → `zone3`. Замена всегда возвращается в **свою** zone, иначе она перекосит locality. Скрипт **пересоздаёт ВМ с тем же именем**; IP обычно новый.
