...
- **WARN**, если нет запаса OS (`memory_limit` > 80% RAM при RAM < 512 GB), prealloc занимает > 90% диска, `log_disk_size` < 3× `memory_limit`, `system_memory` вне диапазона доки, data и clog на одном диске в production.
