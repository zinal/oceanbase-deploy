...
Если под нагрузкой сыпется `-5930` / `maximum open prepared statement handles exceeded` — это уже не роут, а потолок `open_cursors` на сессии observer (дефолт 50, кэш JDBC 2.x — 250): [лимит PS-хендлов](open-cursors.md).
