...
4. Для каждого пакета сначала добавляется `oceanbase-ce`, затем отдельным вызовом — `obagent`, после чего агент явно стартует: `obd cluster start <deploy> -c obagent -s <ip>`. У OBD нет `scale_out`-workflow для obagent, поэтому `scale_out` только ставит пакет и пишет узел в конфиг. Без старта следующий `scale_out` падает на `status_check` (`obagent is not running`).
