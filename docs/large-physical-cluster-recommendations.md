...
4. Для каждого пакета сначала добавляется `oceanbase-ce`, затем отдельным вызовом — `obagent`. Перед `obd cluster start <deploy> -c obagent` каталоги `home_path/{run,bin,lib,conf,log}` создаются на **всех** IP пакета: OBD не вызывает `init` при scale-out obagent, и без `run/` старт заканчивается `fetch_admin_lock_failed`. Нельзя делать `mkdir` только на первом IP и `start -s` по нему: OBD поднимает и остальные новые агенты (или другой server, `-s 10.130.0.8` → `server8`/`10.130.0.18`). Без этого следующего `scale_out` падает на `status_check` (`obagent is not running`).
